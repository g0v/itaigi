...
const debug = Debug('itaigi:Kong其他建議');
...
export default class 其他建議 extends React.Component {
  ...
  顯示建議(建議, i) {
    const 來開例句 = this.props.開例句.bind(this, this.props.華語關鍵字, 建議.文本資料, 建議.音標資料);
    return (
      <一个建議
        key={i}
        文本資料={建議.文本資料}
  ...
  render() {
    const 其他建議 = this.props.內容.其他建議.slice(0, this.state.全開);

    const 文本 = 其他建議.map(this.顯示建議.bind(this));
    if (文本.length == 0) {
      return null;
    }
